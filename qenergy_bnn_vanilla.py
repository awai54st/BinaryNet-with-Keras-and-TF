  ...
  for layer in model.layers:
    ...
    # QBN Layer
      ...
      backprop_rw_energy += backprop_memory_read_energy(
          output_shapes,
          activations_on_memory, min_sram_size, rd_wr_on_io, 32) * batch_size # output activation gradient 
      backprop_rw_energy += backprop_memory_read_energy(
          output_shapes,
          activations_on_memory, min_sram_size, rd_wr_on_io, 32) * batch_size # output activation
      backprop_rw_energy += backprop_memory_write_energy(
          input_shape,
          activations_on_memory, min_sram_size, rd_wr_on_io,
          32) * batch_size # input activation gradients
    ...
    # MAC energy calculation
      ...
      backprop_rw_energy += backprop_memory_read_energy(
          output_shapes,
          activations_on_memory, min_sram_size, rd_wr_on_io, 32) * batch_size # activation gradient (E*_l)
      ...
      if not is_input_layer:
        ...
        backprop_rw_energy += parameter_read_energy(
            layer, layer_item, weights_on_memory, min_sram_size,
            rd_wr_on_io) # weights (sgn(W_l))
        backprop_rw_energy += backprop_memory_write_energy(
            input_shape,
            activations_on_memory, min_sram_size, rd_wr_on_io,
            32) * batch_size # input activation gradients (E_l)
      backprop_rw_energy += backprop_memory_read_energy(
          input_shape,
      ...
      optimizer_rw_energy += backprop_parameter_read_energy(
          layer, layer_item, weights_on_memory, min_sram_size, rd_wr_on_io, 32) # weight (W_l)
      # Optimizer momentum read
      optimizer_rw_energy += 2 * backprop_parameter_read_energy(
          layer, layer_item, weights_on_memory, min_sram_size, rd_wr_on_io, 32) # momentum
